...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    await logEvent({
      actorUserId,
      message: `RESOURCE_CREATE_SUCCESS: NAME=${resourceName}, PRICE=${resourcePrice}€ per ${resourcePriceUnit}, ID=${resourceId}`,
      entityType: "resource",
      entityId: resourceId,
    });
    ...
    if (err && err.code === "23505") {
      ...
      await logEvent({
        actorUserId,
        message: `RESOURCE_CREATE_FAILED_409: DUPLICATE RESOURCE, NAME=${resourceName}`,
        entityType: "resource",
        entityId: null,
      });
