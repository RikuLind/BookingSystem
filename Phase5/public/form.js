...
// Field label map for user-friendly error messages
const fieldLabels = {
  resourceName: "Resource name",
  resourceDescription: "Resource description",
  resourcePrice: "Resource price",
  resourcePriceUnit: "Resource price unit",
  resourceAvailable: "Resource availability"
};
...
function buildValidationMessage(errors) {
  ...
  const lines = errors.map((e) => {
    const field = fieldLabels[e.field] || e.field || "Field";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        ...
        showFormMessage("info", `Duplicate resource:\n\n${msg}`);
        return;
      }
